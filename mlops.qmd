#  {background-image="images/cavemen-wheel.png" background-size="cover" style="font-size: 70%;" align="center"}

::: {.newsection style="--h1-banner-color: #CCCCCCAA; --h1-banner-text-color: #111111"}
MLOps
:::

<!-- ## Technical debt {background-image="images/cavemen-wheel.png"} -->

## Hidden technical debt in ML systems {background-color="#ffffff"}
{{< source "Scully et al. (2015)" url="https://proceedings.neurips.cc/paper_files/paper/2015/file/86df7dcfd896fcaf2674f757a2463eba-Paper.pdf" >}}

```{.d2 theme="Terminal" layout="elk" pad=20 sketch="true"}
**.shape: oval
direction: right
root: Hidden technical debt in ML systems
ddd: Data dependency debt
ad: analysis debt
ap: anti-patterns

root -> ap
ap -> Abstraction debt
ap -> Glue code
ap -> Pipeline jungles
ap -> Dead experimental codepaths
ap -> Common smells

root -> ddd
ddd -> Unstable data dependencies
ddd -> Underutilized data dependencies

root -> ad
ad -> Direct feedback loop
ad -> Indirect feedback loop

root -> configuration debt
```

## Stages of machine learning CI/CD automation pipeline
{{< source "Google Cloud docs" url="https://docs.cloud.google.com/architecture/mlops-continuous-delivery-and-automation-pipelines-in-machine-learning" >}}

![](images/mlops-overview.png){fig-align="center"}

## MLOps level 0 {background-color="#ffffff"}
{{< source "Google Cloud docs" url="https://docs.cloud.google.com/architecture/mlops-continuous-delivery-and-automation-pipelines-in-machine-learning" >}}

![](images/mlops-level-0.svg)

## MLOps level 1 {background-color="#ffffff"}
{{< source "Google Cloud docs" url="https://docs.cloud.google.com/architecture/mlops-continuous-delivery-and-automation-pipelines-in-machine-learning" >}}

![](images/mlops-level-1.svg){fig-align="center"}

## MLOps level 2 {background-color="#ffffff"}
{{< source "Google Cloud docs" url="https://docs.cloud.google.com/architecture/mlops-continuous-delivery-and-automation-pipelines-in-machine-learning" >}}

:::{.notes}
The pipeline consists of the following stages:

1. Development and experimentation: You iteratively try out new ML algorithms and new modeling where the experiment steps are orchestrated. The output of this stage is the source code of the ML pipeline steps that are then pushed to a source repository.
2. Pipeline continuous integration: You build source code and run various tests. The outputs of this stage are pipeline components (packages, executables, and artifacts) to be deployed in a later stage.
3. Pipeline continuous delivery: You deploy the artifacts produced by the CI stage to the target environment. The output of this stage is a deployed pipeline with the new implementation of the model.
4. Automated triggering: The pipeline is automatically executed in production based on a schedule or in response to a trigger. The output of this stage is a trained model that is pushed to the model registry.
5. Model continuous delivery: You serve the trained model as a prediction service for the predictions. The output of this stage is a deployed model prediction service.
6. Monitoring: You collect statistics on the model performance based on live data. The output of this stage is a trigger to execute the pipeline or to execute a new experiment cycle.
:::

![](images/mlops-level-2.svg){fig-align="center"}

## ONNX as the standard for ai interoperability {background-color="#ffffff"}
{{< source "ONNX explained" url="https://viso.ai/computer-vision/onnx-explained-a-new-paradigm-in-ai-interoperability/" >}}

![](images/ONNX-explained.webp)

## The most complete open source MLOps library {background-color="#ffffff"}
{{< source "MLflow" url="https://mlflow.org/docs/latest/ml/" >}}

```{.d2 theme="Terminal" layout="elk" pad=20 width="100%" sketch="true"}
direction: right
mlflow: " " {
  shape: image
  icon: https://raw.githubusercontent.com/mlflow/mlflow/refs/heads/master/assets/logo.svg
}

mlflow -> Tracking \& Experiments
mlflow -> Model Registry
mlflow -> Model Deployment
```

:::{.notes}
MLflow Tracking & Experiments:

- Experiment Organization: Track and compare multiple model experiments
- Metric Visualization: Built-in plots and charts for model performance
- Artifact Storage: Store models, plots, and other files with each run
- Collaboration: Share experiments and results across teams
:::

## MLflow Tracking & Experiments {background-color="#ffffff"}
{{< source "MLflow Tracking" url="https://mlflow.org/docs/latest/ml/tracking/" >}}

![](images/mlflow-tracking-ui.png)

## MLflow Model Registry {background-color="#ffffff"}
{{< source "MLflow Model Registry" url="https://mlflow.org/docs/latest/ml/model-registry/" >}}

![](images/mlflow-registry-overview.png)

## MLflow Model Deployment {background-color="#ffffff"}
{{< source "MLflow" url="https://mlflow.org/docs/latest/ml/deployment/" >}}

![](images/mlflow-deployment.png)

## How about testing? {background-color="#ffffff"}
{{< source "the Practical Test Pyramid" url="https://martinfowler.com/articles/practical-test-pyramid.html" >}}

::: {.columns}
::: {.column}
<br>

![](images/test-pyramid.png){.nostretch fig-align="center" style="display:block; margin-left:auto; margin-right:auto;"}
:::
::: {.column}
<br>

- Write tests with different granularity
- The more high-level you get the fewer tests you should have
:::
:::

## How about testing? {background-color="#ffffff"}
{{< source "Breck et al. (2017)" url="https://research.google/pubs/the-ml-test-score-a-rubric-for-ml-production-readiness-and-technical-debt-reduction/" >}}

![](images/mlops-testing.png)

## The test strategy depends on the end product


::: {.list-table style="font-size: 50%;"}

- - 
  - Type of test

- - **UI**
  - - Relevant when developing a dashboard or app (informatieproduct)
    - End-user testing of interactive visualisations can be very time-consuming and hence costly!

- - **Contract**
  - - Relevant when developing REST APIs
    - Differentiate between consumer vs producer testrow 1 col 2

- - **Integration**
  - - Often starting point of testing e.g. interacting with data stores
    - Consider writing simple checks on datasets when developing ETL pipelines (row counts, number of unique IDs, min-max dates etc.)

- - **Unit**
  - - You shouldn’t need to write unit test if you use standard ML libraries
    - Relevant for data processing (e.g. your own utility functions for data cleaning and data validation (row counts!))
:::


